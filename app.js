...
function onSongComplete() {
  ...
  localStorage.setItem('pb-stars', JSON.stringify(state.stars));
  unlockSticker(id);
  showCelebrate();
  updateStickers();
}
...
function bindCalibration() {
  ...
  fields.forEach((f) => {
    ...
    input.addEventListener('input', () => {
      ...
      val.textContent = formatCalVal(f, state.cal[f]);
      localStorage.setItem('pb-cal', JSON.stringify(state.cal));
  ...
  camFields.forEach((f) => {
    ...
    input.addEventListener('input', () => {
      ...
      val.textContent = state.camCal[f].toFixed(2);
      localStorage.setItem('pb-cam-cal', JSON.stringify(state.camCal));
      drawCameraOverlay([]);
    });
  });
...
function setSpeedMultiplier(mult) {
  ...
  localStorage.setItem('pb-speed-mult', String(mult));
...
function bindControls() {
  ...
  $('#dock-flip-btn')?.addEventListener('click', () => {
    ...
    localStorage.setItem('pb-dock-flip', state.dockFlipH ? '1' : '0');
    applyDockView();
  });

  $('#dock-rotate-btn')?.addEventListener('click', () => {
    state.phoneRotate = !state.phoneRotate;
    localStorage.setItem('pb-phone-rotate', state.phoneRotate ? '1' : '0');
    applyDockView();
    tryWakeLock();
  });
  ...
  $('#phone-rotate-exit')?.addEventListener('click', () => {
    ...
    localStorage.setItem('pb-phone-rotate', '0');
    applyDockView();
  });
  ...
  $('#album-filter')?.addEventListener('change', (e) => {
    ...
    localStorage.setItem('pb-album', state.albumFilter);
    renderSongList();
  });

  $('#album-sort')?.addEventListener('change', (e) => {
    state.albumSort = e.target.value;
    localStorage.setItem('pb-album-sort', state.albumSort);
    renderAlbumControls();
    renderSongList();
  });
}
...
function setRole(role) {
  state.role = role;
  localStorage.setItem('pb-role', role);
  updateRoleUI();
}
...
function loadSharedLesson() {
  ...
  if (lessonParam) {
    try {
      ...
      localStorage.setItem('pb-lessons', JSON.stringify(state.customLessons));
      state.song = lesson;
      if (data.message) {
...
function unlockSticker(songId) {
  ...
  if (!state.stickers.includes(sticker)) {
    ...
    localStorage.setItem('pb-stickers', JSON.stringify(state.stickers));
  }
}
...
setInterval(() => {
  if (state.playing && $('#mirror-screen').classList.contains('active')) {
    drawMirrorKeys(activeNotes(state.song, state.beat));
  }
}, 50);